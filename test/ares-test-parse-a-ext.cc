...
namespace ares {
  namespace test {
    ...
    TEST_F(LibraryTest, ParseAReplyExtOK) {
      DNSPacket pkt;
      ...
      int count = 5;
      EXPECT_EQ(ARES_SUCCESS, ares_parse_a_reply_ext(data.data(), data.size(),
                                                     &host, info, &count));
      EXPECT_EQ(1, count);
      ...
      std::stringstream ss;
      ss << HostEnt(host);
      EXPECT_EQ("{'example.com' aliases=[] addrs=[]}", ss.str());
    ...
    TEST_F(LibraryTest, ParseMalformedAReplyExt) {
      ...
      EXPECT_EQ(ARES_EBADRESP, ares_parse_a_reply_ext(invalid_rrlen.data(), invalid_rrlen.size(),
                                                      &host, info, &count));

      // Truncate mid-question.
      EXPECT_EQ(ARES_EBADRESP, ares_parse_a_reply_ext(data.data(), 26,
                                                      &host, info, &count));

      // Truncate mid-answer.
      EXPECT_EQ(ARES_EBADRESP, ares_parse_a_reply_ext(data.data(), 42,
                                                      &host, info, &count));
    }

    TEST_F(LibraryTest, ParseAReplyExtNoData) {
      DNSPacket pkt;
      pkt.set_qid(0x1234).set_response().set_aa()
        .add_question(new DNSQuestion("example.com", ns_t_a));
      std::vector<byte> data = pkt.data();
      ...
      struct ares_addrttl info[2];
      int count = 2;
      EXPECT_EQ(ARES_ENODATA, ares_parse_a_reply_ext(data.data(), data.size(),
                                                     &host, info, &count));
      EXPECT_EQ(0, count);
      ...
      // Again but with a NS record
      pkt.add_auth(new DNSNsRR("example.com", 292, "ns1.example.com"));
      EXPECT_EQ(ARES_ENODATA, ares_parse_a_reply_ext(data.data(), data.size(),
                                                     &host, info, &count));
      EXPECT_EQ(0, count);
      EXPECT_EQ(nullptr, host);
    ...
    TEST_F(LibraryTest, ParseAReplyExtVariantA) {
      DNSPacket pkt;
      ...
      ss << HostEnt(host);
      EXPECT_EQ("{'mit.edu' "
              "aliases=[W20NS.mit.edu, BITSY.mit.edu, STRAWB.mit.edu] "
              "addrs=[]}", ss.str());

      ares_free_hostent(host);
    }

    TEST_F(LibraryTest, ParseAReplyExtVariantCname) {
      DNSPacket pkt;
      pkt.set_qid(6366).set_rd().set_ra()
      ...
      std::stringstream ss;
      ss << HostEnt(host);
      EXPECT_EQ("{'redirect.query.example.com' "
              "aliases=[aa.ns1.example.com, ns2.example.com, ns3.example.com, ns4.example.com] "
              "addrs=[]}", ss.str());
    ...
    TEST_F(LibraryTest, ParseAReplyExtVariantCnameChain) {
      DNSPacket pkt;
      pkt.set_qid(6366).set_rd().set_ra()
    ...
    TEST_F(LibraryTest, DISABLED_ParseAReplyExtVariantCnameLast) {
      DNSPacket pkt;
      pkt.set_qid(6366).set_rd().set_ra()
        .add_question(new DNSQuestion("query.example.com", ns_t_a))
    ...
    TEST_F(LibraryTest, ParseAReplyExtErrors) {
      DNSPacket pkt;
      pkt.set_qid(0x1234).set_response().set_aa()
        .add_question(new DNSQuestion("example.com", ns_t_a))
      ...
      data = pkt.data();
      EXPECT_EQ(ARES_EBADRESP, ares_parse_a_reply_ext(data.data(), data.size(),
                                                      &host, info, &count));
      EXPECT_EQ(nullptr, host);
  ...
  pkt.set_rcode(ns_r_noerror);
      ...
      for (size_t len = 1; len < data.size(); len++) {
        EXPECT_EQ(ARES_EBADRESP, ares_parse_a_reply_ext(data.data(), len,
                                                        &host, info, &count));
        EXPECT_EQ(nullptr, host);
        EXPECT_EQ(ARES_EBADRESP, ares_parse_a_reply_ext(data.data(), len,
                                                        nullptr, info, &count));
    ...
    TEST_F(LibraryTest, ParseAReplyExtAllocFail) {
      DNSPacket pkt;
      pkt.set_qid(0x1234).set_response().set_aa()
        .add_question(new DNSQuestion("example.com", ns_t_a))
        .add_answer(new DNSCnameRR("example.com", 300, "c.example.com"))
        .add_answer(new DNSARR("c.example.com", 500, {0x02, 0x03, 0x04, 0x05}))
        .add_auth(new DNSNsRR("example.com", 299, "ns.example.com"));
      std::vector<byte> data = pkt.data();
      ...
      for (int ii = 1; ii <= 10; ii++) {
        ClearFails();
        SetAllocFail(ii);
        EXPECT_EQ(ARES_ENOMEM, ares_parse_a_reply_ext(data.data(), data.size(),
                                                      &host, info, &count)) << ii;
        EXPECT_EQ(nullptr, host);
      }
    }
